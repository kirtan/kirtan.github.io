...
(function() {
  ...
  $(function() {
    ...
    make_dot = function(args) {
      if (args.outer_padding == null) {
        args.outer_padding = 0;
      }
      if (args.outer_opacity == null) {
    ...
    set_goals = function(points) {
      var i, _base, _base1, _base2, _base3, _base4, _i, _ref, _results;
      _results = [];
      for (i = _i = 0, _ref = points.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        if ((_base = points[i]).z_index == null) {
          _base.z_index = 1;
        }
        if ((_base1 = points[i]).outer_padding == null) {
          _base1.outer_padding = 0;
        }
        if ((_base2 = points[i]).outer_opacity == null) {
          _base2.outer_opacity = 0;
        }
        if ((_base3 = points[i]).absolute == null) {
          _base3.absolute = false;
        }
        if ((_base4 = points[i]).immediate == null) {
          _base4.immediate = [];
        }
    ...
    sharing = function(position) {
      ...
      moving_points = (function() {
        ...
        for (i = _i = 0; 0 <= size ? _i < size : _i > size; i = 0 <= size ? ++_i : --_i) {
          _results.push({
            ang: 2 * Math.PI * i / size + random(),
            velocity: Math.sqrt(Math.random() + .2) * .015 * (random() > 0 ? 1 : -1),
            orbit: random() + radius,
            color: color,
            radius: (.1 + random()) * .3,
      ...
      return {
        get_points: function(time, dtim, elapsed_time) {
          var ang, distance, i, near_pin, p, points, _i, _j, _len;
          points = [];
          ...
          for (i = _j = 0, _len = moving_points.length; _j < _len; i = ++_j) {
            ...
            ang = (elapsed_time + 5000 * i / moving_points.length) > 5500 ? (p.opacity += random() * .2, distance = Math.abs(Math.abs(near_pin - parseInt(near_pin)) - .5) * 4, p.ang += dtim / 10 * p.velocity * .8 / (distance * distance * distance + .1), p.ang) : parseInt(near_pin) / pins * 2 * Math.PI - Math.PI / 32;
            p.position = {
    ...
    tree = function() {
      ...
      initialize_child = function(time, parent, child, pivot, left) {
        ...
        child.position = {
          x: parent.position.x + (Math.random() * .3 + .7) * (left ? -1 : 1) / (child.level * child.level),
